...
bazel-server-start: bazel-image ## Starts the bazel server.
	...
	docker run -d --rm --name $(DOCKER_NAME) \
		...
		$(BUILDER_IMAGE) \
		sh -c "echo \$$(bazel info server_pid) && tail -f --pid=\$$(bazel info server_pid) /dev/null"
	docker ps
	docker exec $(DOCKER_NAME) true
	docker exec $(FULL_DOCKER_EXEC_OPTIONS) $(DOCKER_NAME) true
...
build_paths = command_line=$$( $(build_cmd) 2>&1 \
		...
		| xargs -n 1 -I {} echo "$(1)" ) && \
		(set -xeuo pipefail; echo $${command_line}; eval $${command_line})
